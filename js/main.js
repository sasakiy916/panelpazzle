...
document.addEventListener("DOMContentLoaded",()=>{
    const size=4;//盤面の大きさ
    const difficulty=20;//ゲームの複雑さ
    const shuffleCount=size*difficulty;//シャッフル回数
    ...
    startBt.addEventListener("click",()=>{
        ...
        for(let i=0;i<shuffleCount;i++){
            const dir = Math.floor(Math.random() * 4);//0:上,1:右,2:下,3左:
            switch(dir){
                case 0:
    ...
    const click =(e)=>{
        ...
        if(pos>=size && panels[pos-size].textContent===""){//上が空白か
            swap(pos,pos-size);
        }else if((pos+1)%size !== 0 && panels[pos+1].textContent===""){//右
            swap(pos,pos+1);
        }else if(pos < size*(size-1) && panels[pos+size].textContent===""){//下
            swap(pos,pos+size);
        }else if(pos%size !== 0 && panels[pos-1].textContent===""){//左
            swap(pos,pos-1);
        }
        check();
    };
